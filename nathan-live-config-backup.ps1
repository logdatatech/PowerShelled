...
#$switch_model_config_retrieve_type = "Cisco_Default"

$live_switch = "CAM000002CS001"
#$switch_model_config_retrieve_type = "Cisco_Default"
$switch_model_config_retrieve_type = "Nortel_Default"

write-host "Attempting to Retrieve Live Config from ... $live_switch.comms.mil.au"

# need a Switch Case for the Config Set of Each Switch!
switch ($switch_model_config_retrieve_type) 
    { 
        "Cisco_Default" {Get-Telnet -RemoteHost "$live_switch.comms.mil.au" -Commands " ",$tacacs_username,$passwordPlainTextString,"en","terminal length 0","show running-config" -OutputPath "G:\JP2047 _Telstra\RTC.Tasks\8420.07.VLANS\Live.Config.Extracts\$live_switch.cfg" -WaitTime 1500}
        "Nortel_Default" {Write-Host "Command Set is Blank!"} 
        default {Write-Host "The Command Set To Retrieve Switch Config Is Not Defined!"}
    }

#Get-Telnet -RemoteHost "$live_switch.comms.mil.au" -Commands $tacacs_username,$passwordPlainTextString,"en","terminal length 0","show running-config" -OutputPath "G:\JP2047 _Telstra\RTC.Tasks\8420.07.VLANS\Live.Config.Extracts\$live_switch.cfg" -WaitTime 2000
